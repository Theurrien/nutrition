...
import { Component, ComponentSet, Food, GetFoodValuesParams, Language, Value } from '../api/types.js';
 ...
 */
export async function getComponentSets(language: Language = DEFAULT_LANGUAGE): Promise<ComponentSet[]> {
  try {
 ...
 */
export async function getComponents(language: Language = DEFAULT_LANGUAGE): Promise<Component[]> {
  try {
...
export async function getNutritionalValues(params: {
  ...
  try {
    ...
    const valuesParams: GetFoodValuesParams = {
      DBID: dbid,
    ...
    if (!values || values.length === 0) {
      throw new McpError(
        ...
        `No nutritional values found for food ID ${foodId} with component set ID ${componentSetId}`
      );
    }
...
export async function compareNutritionalValues(params: {
  ...
  foods: Food[];
  comparisonTable: Record<string, Array<{ value: number; formattedValue: string }>>;
}> {
  ...
  if (!foodIds || foodIds.length < 2) {
    throw new McpError(
      ...
      'At least two food IDs are required for comparison'
    );
  }
  ...
  try {
    ...
    const valuesPromises = foodIds.map(foodId => 
      getNutritionalValues({ foodId, componentSetId, language })
    );
    const valuesArrays = await Promise.all(valuesPromises);
    
    // Build comparison table
    const comparisonTable: Record<string, Array<{ value: number; formattedValue: string }>> = {};
    ...
    valuesArrays.forEach(valueArray => {
      ...
      const componentValueMap = new Map<string, Value>();
      valueArray.forEach(value => {
