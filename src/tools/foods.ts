...
import { Food, FoodWithNamesSynonymesCategories, Language, SearchFoodsParams } from '../api/types.js';
  ...
  try {
    ...
    if (!food) {
      throw new McpError(
        ...
        translate('error.notFound', language)
      );
    }
 ...
 */
export async function getTopCategories(language: Language = DEFAULT_LANGUAGE) {
  try {
