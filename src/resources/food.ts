 ...
 */
export async function handleFoodResource(uri: string, language: Language = DEFAULT_LANGUAGE) {
 ...
 */
async function handleFoodDetails(foodId: number, language: Language) {
  try {
    ...
    if (error instanceof Error) {
      throw new McpError(
        ...
        `Error retrieving food details: ${error.message}`
      );
    }
 ...
 */
async function handleFoodSearch(query: string, language: Language) {
  try {
    ...
    if (error instanceof Error) {
      throw new McpError(
        ErrorCode.InternalError,
        `Error searching foods: ${error.message}`
      );
    }
 ...
 */
async function handleFoodCategory(categoryId: number, language: Language) {
  try {
    ...
    if (error instanceof Error) {
      throw new McpError(
        ErrorCode.InternalError,
        `Error retrieving foods in category: ${error.message}`
      );
    }
