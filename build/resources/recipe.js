 ...
 */
export async function handleRecipeResource(uri, language = DEFAULT_LANGUAGE) {
    ...
    throw new McpError(ErrorCode.InvalidRequest, `Invalid recipe resource URI: ${uri}`);
}
/**
 ...
 */
async function handleRecipeDetails(recipeId, language) {
    try {
        ...
        if (error instanceof Error) {
            throw new McpError(ErrorCode.InternalError, `Error retrieving recipe details: ${error.message}`);
        }
        throw error;
    }
}
 ...
 */
async function handleRecipeIngredients(recipeId, language) {
    try {
        ...
        if (error instanceof Error) {
            throw new McpError(ErrorCode.InternalError, `Error retrieving recipe ingredients: ${error.message}`);
        }
        throw error;
    }
}
 ...
 */
async function handleRecipeNutrition(recipeId, language) {
    try {
        ...
        if (error instanceof Error) {
            throw new McpError(ErrorCode.InternalError, `Error calculating recipe nutrition: ${error.message}`);
        }
        throw error;
    }
}
